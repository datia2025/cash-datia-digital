...
# The 96 payloads built directly (mirroring the SQL generation logic)
# to avoid dependency on parsing the SQL file which can be fragile.
...
def build_payloads():
    ...
    for year in YEARS:
        ...
        if not os.path.exists(json_file):
            print(f"AVISO: Archivo no encontrado: {json_file}")
            continue
        ...
        for q in range(1, 5):
            ...
            for key in INDICATORS:
                ...
                payloads.append({
                    ...
                    "titulo": f"Bloque C: {key.upper()} - {period_name} {year}",
                    "analisis_positivo": pos,
                    "analisis_negativo": neg,
                    "recomendacion": rec,
                    "metodologia": f"Bloque C - Rentabilidad - Gerencial {year}",
...
def inject_batch(payloads, batch_size=5, delay=8, inter_delay=2.5):
    print(f"Inyectando {len(payloads)} registros del Bloque C - Rentabilidad Gerencial...")
    ok = 0
    errors = 0
    for i in range(0, len(payloads), batch_size):
        batch = payloads[i:i + batch_size]
        print(f"\n--- Lote {i//batch_size + 1} ({len(batch)} registros) ---")
        for rec in batch:
            code, resp = post_record(rec)
            label = f"{rec['indicador_key']} {rec['periodo_ano']} {rec['period_key']}"
            if code == 200:
                print(f"  OK : {label}")
                ok += 1
            else:
                print(f"  ERR {code}: {label} -> {resp[:80]}")
                errors += 1
            time.sleep(inter_delay)
        if i + batch_size < len(payloads):
            print(f"  Pausa de seguridad: {delay}s...")
            time.sleep(delay)
    print(f"\nFinalizado. OK: {ok} | Errores: {errors} | Total: {ok+errors}")


if __name__ == "__main__":
    payloads = build_payloads()
    print(f"Payloads construidos: {len(payloads)}")

"""
Generador de CSVs para módulo SOLVENCIA
Usa la misma lógica DAX-aligned de calculate_indicators.py
Output: acid test/GENERADOS/SOLVENCIA/
"""
import sys, os, csv
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'sources'))
from calculate_indicators import *

OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'GENERADOS', 'SOLVENCIA')
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("=" * 60)
print("GENERADOR — Módulo SOLVENCIA")
print("=" * 60)
...
# Indicators that use full precision (no ROUND in DAX output)
FULL_PRECISION = {'Endeudamiento Total'}

print(f"\n[7] Writing {len(SOLVENCIA_INDICATORS)} CSVs to {OUTPUT_DIR}")

for indicator_name, header in SOLVENCIA_INDICATORS.items():
    ...
    with open(filepath, 'w', newline='', encoding='utf-8') as f:
        ...
        for q, val, period, year in data:
            if isinstance(val, float):
                if indicator_name in FULL_PRECISION:
                    val_fmt = val  # Full precision
                else:
                    rounded = round(val, 2)
...
print(f"\n{'=' * 60}")
print(f"GENERADOS: {len(SOLVENCIA_INDICATORS)} archivos en {OUTPUT_DIR}")
print(f"{'=' * 60}")
